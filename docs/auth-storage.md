...
│  UserStore                                        │
│  BlobStore  (binary files, any purpose)           │
└──────────────────┬───────────────────────────────┘
                   │ implemented by
           ┌───────┴───────┐            ┌──────────┐
           ▼                ▼            ▼          ▼
┌──────────────────┐  ┌───────────┐  ┌──────────────────┐
│ KvUserStore      │  │ InMemUser │  │ KvBlobStore      │
│ (kv-store.ts)    │  │ Store     │  │ (kv-store.ts)    │
│ Deno KV          │  │ (Map)     │  │ kv-toolbox       │
│ durable          │  │ ephemeral │  │ chunking         │
└──────┬───────────┘  └───────────┘  └──────┬───────────┘
       │                                     │
       └──────────────┬──────────────────────┘
                      ▼
...
Each auth module owns its config namespace. The form-auth module reads:
...
```sh
BLENNY_FORM_AUTH_STORE=memory deno run main.ts
```
...
The SurrealDB auth module drops into `modules/` — no changes to `main.ts`,
`config.ts`, or `app-state.ts`:

```ts
const surrealdbAuthModule: BlennyModule = {
  name: "surrealdb-auth",
  capabilities: ["auth"],

  async initialize(state: AppState) {
    const db = requireDb(state.db, "surrealdb-auth");
    store = new SurrealUserStore(db);
...
The in-memory store backs all existing tests — no flag changes needed in the
standard `deno test` task.

New KV store tests use `openKvStore("")` to create an isolated in-memory KV
instance per test:
